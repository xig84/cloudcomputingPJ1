...
public class part3P8 {

    public static class DateMapper extends Mapper<LongWritable, Text, Text, IntWritable> {

        private final static IntWritable bytes = new IntWritable();
        private Text date = new Text();

        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            String line = value.toString();
            if (line.contains("19/Dec/2020")) {
                String[] parts = line.split("\"");
                if (parts.length >= 3) {
                    String[] sizeParts = parts[2].split("\\s+");
                    if (sizeParts.length >= 2) {
                        try {
                            bytes.set(Integer.parseInt(sizeParts[1]));
                            date.set("19/Dec/2020");
                            context.write(date, bytes);
                        } catch (NumberFormatException e) {
                            // Ignore lines where size is not a number
                        }
                    }
                }
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        job.setJarByClass(part3P8.class);
